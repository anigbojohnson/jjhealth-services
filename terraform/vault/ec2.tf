    ...
    Statement = [{       
      ...
      "Resource":[ 
        "arn:aws:secretsmanager:${var.region}:${data.aws_caller_identity.current.account_id}:secret:${var.vault_secret_name_root_token}*",
          ...
          "arn:aws:secretsmanager:${var.region}:${data.aws_caller_identity.current.account_id}:secret:${var.vault_secret_name_root_certificate_authority}*"
