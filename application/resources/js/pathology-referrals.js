  ...
  window.goToNext = function () {
    if (currentStep < steps.length - 1) {
      ...
      renderStep();
       ...
       window.location.href = "/pathology/select";
    }
  };
...
$(document).ready(function () {
    ...
    $('#personal-detail-form').on('submit', function (e) {
        ...
        $.ajax({
            url: "/pathology-referral-personal-details",
            method: 'POST',
    ...
    $('#consultation-pathology-refferals-form').on('submit', function (e) {
      ...
      e.preventDefault();
        ...
        if (!$('#medicalConditionImage').val()) {
            $('#medicalConditionImage-error').text('Please select Yes or No.');
            isValid = false;
        }

        if ($('#medicalConditionImage').val() === 'Yes' && !$('#fileUpload').val()) {
            $('#fileUpload-error').text('Please upload an image.');
            isValid = false;
        }

        if ($('input[name="selected_tests[]"]:checked').length === 0) {
            $('#pathology-listed-error').text('Please select at least one test before proceeding.');
            isValid = false;
          }
        if(!isValid){
          return
    ...
    $('#consult').prop('disabled', true).text('Processing...');
        ...
        $.ajax({
            url: "/pathology-refferals-consultation-details",
            method: 'POST',
            data: new FormData(this),
            dataType: 'json',
            processData: false,
    ...
    $('#back-personal-details').on('click', function () {
      ...
      // Clear medicalConditionImage error when Yes/No is clicked
$('.option-btn[data-target="medicalConditionImage"]').on('click', function() {
    $('#medicalConditionImage-error').text('');

    // If No is selected, clear file upload error too
    if ($(this).data('value') === 'No') {
        $('#fileUpload-error').text('');
    }
});

// Clear fileUpload error when a file is selected
...
// Clear pathology error when any checkbox is checked
$('input[name="selected_tests[]"]').on('change', function() {
    if ($('input[name="selected_tests[]"]:checked').length > 0) {
        $('#pathology-listed-error').text('');
    } else {
        $('#pathology-listed-error').text('Please select at least one test before proceeding.');
    }
});
